...
namespace PrikolShopBusinessLogic.BusinessLogics
{
    static class SaveToExcel
    {
        public static void CreateDoc(ExcelInfo info)
        {
            ...
            {                 
                ...
                foreach (var box in info.Boxes)
                {
                    ...
                    foreach (var giftbox in box.GiftBoxes)
                    {
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            Worksheet = worksheetPart.Worksheet,
        ...
        private static void InsertCellInWorksheet(ExcelCellParameters cellParameters)
        {
            ...
            {
                // Все ячейки должны быть последовательно друг за другом расположены                 // нужно определить, после какой вставлять
                Cell refCell = null;
